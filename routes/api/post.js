...
router.put('/like/:id', auth, async (req, res) => {
  try {
    const post = await Post.findById(req.params.id);
    if (
      post.likes.filter(like => like.user.toString() === req.user.id && like.like === "like").length > 0
    ) {
      return res.status(400).json({ msg: 'post already liked' });
    }
    if (post.likes.filter(like => like.like === "unlike").length > 0) {
      ...
      post.likes.unshift({ user: req.user.id, like: "like" });
    }
    await post.save();
    post.likes.filter((like) => like.like === "like");
    res.json(post.likes);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server error');
...
router.put('/unlike/:id', auth, async (req, res) => {
  try {
    const post = await Post.findById(req.params.id);
    if (
      post.likes.filter(like => like.user.toString() === req.user.id && like.like === "unlike").length >
      0
    ) {
      return res.status(400).json({ msg: 'post already liked' });
    } 
    if (post.likes.filter(like => like.like === "like").length > 0){
      let removeIndex = post.likes
      .map(like => like.user.toString())
      .indexOf(req.user.id);
      ...
      post.likes.unshift({ user: req.user.id, like: "unlike" });
    }

    await post.save();
    post.likes.filter((like) => like.like === "unlike");
    res.json(post.likes);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server error');
